...
(
    {
        @Validation( rule = "${ Max >= Min }", message = "Max must not be smaller than min." ),
        @Validation( rule = "${ Max <= 100 }", message = "Max must be less than or equal to 100.", severity = Status.Severity.WARNING )
    }
)

public interface TestElementForElement extends IModelElement
{
    ModelElementType TYPE = new ModelElementType( TestElementForElement.class );
